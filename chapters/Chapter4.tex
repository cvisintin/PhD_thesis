...
\begin{figure*}[!t]
  \centering
  \includegraphics[scale=.48]{04_study_vic2.png}\\
  \includegraphics[scale=.48]{04_study_cal2.png}
...
\noindent where $p_i=\text{Pr}(Y_i=1)$ is the relative likelihood of a collision occurring on a road segment $i$, $O_i$ is species occurrence, $V_i$ is traffic volume, $S_i$ is traffic speed, $L_i$ is an offset term for road segment length and $T$ is the time span of collision training data in years. We chose the complementary log-log link on the linear predictor due to the mathematical theory underpinning our model -- risk being measured by the rate of collisions (see \Cref{sec:egk}).

To test for spatial autocorrelation, we calculated normally-distributed, randomised quantile residuals \citep[see][]{dunn96} using 5,000 simulations from each model fit. We then calculated Moran's I on the residuals at 20 spatial lags at intervals of both one kilometre and 250 metres.  For each spatial resolution, we repeated the Moran's I calculation twenty times for 5,000 randomly sampled road segments (due to computational limitations) and plotted the twenty trend lines for visual inspection at both spatial scales (\Cref{cal_sac}).

\begin{figure*}[!h]
  \captionsetup[subfloat]{farskip=-2pt,nearskip=-2pt}
  \centering
  \subfloat[Eastern Grey Kangaroos]{\label{cal_sac:a}\includegraphics[height=7.5cm]{04_vic_coll_cor_1000.png}}
  \subfloat[Mule Deer]{\label{cal_sac:b}\includegraphics[height=7.5cm]{04_cal_coll_cor_1000_2.png}}\\[1cm]
  \subfloat[Eastern Grey Kangaroos]{\label{cal_sac:c}\includegraphics[height=7.5cm]{04_vic_coll_cor_250.png}}
  \subfloat[Mule Deer]{\label{cal_sac:d}\includegraphics[height=7.5cm]{04_cal_coll_cor_250_2.png}}
...
The two models were used to predict relative collision risk on all road segments in each respective study area (\Cref{cal_coll_preds}). The collision risks were expressed as expected annual collisions per kilometre on each road segment. An analyst or manager may use this information to determine: (a) relative risk on roads by summing the segment values and comparing to other problematic roads; (b) locations to prioritise mitigation; or (c) locations for additional investigation such as monitoring.
...
\begin{figure*}[!t]
  \centering
  \includegraphics[scale=.47]{04_egk_collrisk_vic2.png}\\
  \includegraphics[scale=.47]{04_deer_collrisk_cal2.png}
...
\begin{figure*}[!t]
  \captionsetup[subfloat]{farskip=-2pt,nearskip=-2pt}
  \centering
  \subfloat[Eastern Grey Kangaroos]{\label{cal_coll_effects:a}\includegraphics[width=0.4\textwidth]{04_vic_occ.png}}
  \subfloat[Mule Deer]{\label{cal_coll_effects:b}\includegraphics[width=0.4\textwidth]{04_cal_occ.png}}\\
  \subfloat[Eastern Grey Kangaroos]{\label{cal_coll_effects:c}\includegraphics[width=0.4\textwidth]{04_vic_tvol.png}}
  \subfloat[Mule Deer]{\label{cal_coll_effects:d}\includegraphics[width=0.4\textwidth]{04_cal_tspd.png}}\\
  \subfloat[Eastern Grey Kangaroos]{\label{cal_coll_effects:e}\includegraphics[width=0.4\textwidth]{04_vic_tvol.png}}
  \subfloat[Mule Deer]{\label{cal_coll_effects:f}\includegraphics[width=0.4\textwidth]{04_cal_tspd.png}}
...
The predictions for relative collision risk across all road segments were plausible in most regions of the two study areas (\Cref{cal_coll_preds}).  In Victoria, predicted collision risk was highest on segments along the Hume Highway to the north of Melbourne -- where collisions frequently occur -- and on segments along the Princes Highway to the east of Melbourne. In contrast, predictions in California were highest on road segments along Interstate 280 in the San Francisco Bay Area, where deer collisions are common, and along Highway 50 in the foothills of the Sierra Nevada mountain range. \Cref{rd_seg_risk} lists the road segments with the highest predicted risk in both study areas.
...
\begin{figure*}[!h]
  \captionsetup[subfloat]{farskip=-2pt,nearskip=-2pt}
  \centering
  \subfloat[Eastern Grey Kangaroos]{\label{cal_occ_effects:a}\includegraphics[width=0.45\textwidth]{04_vic_light.png}}
  \subfloat[Mule Deer]{\label{cal_occ_effects:b}\includegraphics[width=0.45\textwidth]{04_cal_slope.png}}\\
  \subfloat[Eastern Grey Kangaroos]{\label{cal_occ_effects:c}\includegraphics[width=0.45\textwidth]{04_vic_elev.png}}
  \subfloat[Mule Deer]{\label{cal_occ_effects:d}\includegraphics[width=0.45\textwidth]{04_cal_light.png}}\\
  \subfloat[Eastern Grey Kangaroos]{\label{cal_occ_effects:e}\includegraphics[width=0.45\textwidth]{04_vic_green.png}}
  \subfloat[Mule Deer]{\label{cal_occ_effects:f}\includegraphics[width=0.45\textwidth]{04_cal_green.png}}
...
\begin{figure*}[!h]
  \centering
  \includegraphics[scale=.47]{04_egk_preds_vic.png}\\
  \includegraphics[scale=.47]{04_deer_preds_cal.png}
